...
namespace Doom
{
    ...
    */
    public class SceneComponent : Component
    {
        ...
        protected bool IsLogoVisible = true;
        //{
        ...
        protected void SimpleCreateInstructions(string text = "", Color textColor = new Color())
        {
            ...
            //}
            //
            // Remove child and reAdd (same as clear the text)
            //
            if (infoText != null)
                Application.UI.Root.RemoveChild(infoText);
        ...
        /// <summary>
        /// Save the scene for later load
        /// </summary>
        /// <param name="_path : Assets/Data/Scenes is default"></param>
        /// <param name="_sceneName : SceneName.xml"></param>
        /// <returns></returns>
        public bool SaveScene(string _path, string _sceneName)
        {
            if (_path.Length == 0)
                _path = FileSystem.CurrentDir + "Assets/Data/Scenes";
            if (!FileSystem.DirExists(_path))
            {
                FileSystem.CreateDir(_path);
            }
            return MyScene.SaveXml(_path + "/" + _sceneName);
        }
        /// <summary>
        /// Load a saved scene
        /// </summary>
        ...
        /// <param name="_sceneName : SceneName.xml"></param>
        /// <returns></returns>
        public bool LoadScene(string _path, string _sceneName)
        {
            if (_path.Length == 0)
                _path = FileSystem.CurrentDir + "Assets/Data/Scenes";
            if (FileSystem.DirExists(_path))
            {
                return MyScene.LoadXml(_path + "/" + _sceneName);
            }
            return false;
        }
        ...
        public virtual void HandleKeyDown(KeyDownEventArgs e)
        {
            switch (e.Key)
            {
                case Key.F1:
